...
export default function Attendance() {
  ...
  useEffect(() => {
    ...
    setSearchParams((prev) => {
      ...
      next.set('subject_id', selectedCourse);
      return next;
    });
  ...
  async function loadSessions(subjectId) {
    try {
      const results = await getProfessorSessions(subjectId);
      setSessions(results);
      if (results.length > 0) {
  ...
  async function handleExport(format) {
    ...
    try {
      ...
      if (format === 'pdf') {
        exportToPDF(rows, 'Session Attendance Report', columns, 'session-attendance', meta);
      } else {
        exportToExcel(rows, columns, 'session-attendance');
      }
    ...
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-[var(--text-base)] text-[var(--color-text-secondary)]">Filter sessions and manage attendance records.</p>
        </div>
            ...
            <div>
              ...
              <Select
                ...
                options={[
                  ...
                  ...courses.map((course) => ({
                    value: course.id,
                    label: `${course.name_en} (${course.code || course.subject_code || 'N/A'})`,
                  })),
                ]}
              />
            </div>
