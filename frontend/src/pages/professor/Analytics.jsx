...
export default function Analytics() {
  ...
  const [selectedCourse, setSelectedCourse] = useState(searchParams.get('subject_id') || '');
  ...
  async function loadCourses() {
    try {
      ...
      if (results.length > 0 && !selectedCourse) {
        setSelectedCourse(results[0].id);
      }
    } catch {
  ...
  useEffect(() => {
    ...
    if (selectedCourse) next.set('subject_id', selectedCourse);
  ...
  }, [selectedCourse, dateFrom, dateTo, searchParams, setSearchParams]);

  useEffect(() => {
    if (!selectedCourse) return;
    generateReport({ subjectId: selectedCourse });
  }, [selectedCourse]);

  async function generateReport(nextFilters = {}) {
    const subjectId = nextFilters.subjectId ?? selectedCourse;
  ...
  function exportRows(format) {
    ...
    const meta = {
      Subject: selectedCourseMeta?.name_en || 'N/A',
      DateFrom: dateFrom || 'All',
      DateTo: dateTo || 'All',
    };

    if (format === 'pdf') {
      exportToPDF(rows, 'Professor Analytics Report', columns, 'professor-analytics', meta);
    } else {
      exportToExcel(rows, columns, 'professor-analytics');
    }
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-[var(--text-3xl)] font-bold text-[var(--color-text-primary)]">Reports & Analytics</h1>
          <p className="text-[var(--text-base)] text-[var(--color-text-secondary)]">Generate filtered attendance insights per subject and date range.</p>
        </div>
      </header>
            ...
            <div>
              ...
              <Select
                ...
                options={[
                  ...
                  ...courses.map((course) => ({
                    value: course.id,
                    label: `${course.name_en || course.name_ar || 'Untitled'} (${course.code || 'N/A'})`,
                  })),
                ]}
              />
            </div>
            ...
            <div className="md:col-span-2">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={() => {
                    const fallbackSubject = courses[0]?.id || '';
                    setSelectedCourse(fallbackSubject);
                    setDateFrom('');
                    setDateTo('');
                    if (fallbackSubject) {
