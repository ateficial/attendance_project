...
import { getTADashboardStats } from '../../lib/apiClient';
...
export default function TAOverview() {
  ...
  const loadDashboard = async () => {
    try {
      const payload = await getTADashboardStats();
      setStats(payload);
      setError('');
    } catch (e) {
    ...
    datasets: [
      {
        ...
        backgroundColor: (ctx) => {
          ...
          if (!area) return 'rgba(8,145,178,0.15)';
          const gradient = chart.ctx.createLinearGradient(0, area.top, 0, area.bottom);
          gradient.addColorStop(0, 'rgba(8,145,178,0.28)');
          gradient.addColorStop(1, 'rgba(8,145,178,0.03)');
          return gradient;
        },
      },
    ],
  ...
  return (
    <div className="space-y-6">
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-[var(--text-3xl)] font-bold text-[var(--color-text-primary)]">TA Dashboard</h1>
          <p className="text-[var(--text-base)] text-[var(--color-text-secondary)]">Realtime attendance overview across your sections.</p>
        </div>
      ...
      <Card variant="elevated">
        <CardHeader>
          <h2 className="text-[var(--text-lg)] font-semibold text-[var(--color-text-primary)]">Sections Summary</h2>
        </CardHeader>
        <CardBody className="p-0">
          <DataTable
            ...
            rows={summaryRows}
            rowKey="subject_id"
            onRowClick={(row) => navigate(`/ta/attendance?subject_id=${encodeURIComponent(row.subject_id)}`)}
            emptyIcon="analytics"
      ...
      <Card variant="elevated">
        ...
        <div className="space-y-2">
          {atRiskRows.map((row, index) => {
            const initials = String(row.name || 'TA')
              .split(' ')
              .filter(Boolean)
              .slice(0, 2)
